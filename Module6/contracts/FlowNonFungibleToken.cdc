...
pub contract interface NonFungibleToken {
    ...
    // Total count of tokens in existence
    pub var totalSupply: UInt64

    // Event emitted upon contract initialization
    ...
    pub resource interface Provider {
        pub fun withdraw(withdrawID: UInt64): @NFT {
            post {
                result.id == withdrawID: "The ID of the withdrawn token must align with the requested ID"
            }
        }
    }
    ...
    pub resource Collection: Provider, Receiver, CollectionPublic {
        ...
        pub fun borrowNFT(id: UInt64): &NFT {
            pre {
                self.ownedNFTs[id] != nil: "The NFT does not exist!"
            }
        }
    }
    ...
    pub fun createEmptyCollection(): @Collection {
        post {
            result.getIDs().length == 0: "The collection must be devoid of any elements!"
        }
    }
}
